  ...
  return (
    <section id="experience" className="relative border-t border-white/5 bg-background py-28">
      <div className="mx-auto max-w-7xl px-6">
        <SectionHeading
          label="// Traversal Network"
          title="Experience as a route across systems."
        />
        <div className="relative">
          <motion.div
            ...
            style={{ originY: 0 }}
            className="absolute left-4 top-0 h-full w-px bg-gradient-to-b from-primary via-primary/60 to-transparent md:left-1/2 md:-translate-x-1/2"
          />
              ...
              return (
                <li key={i} className="relative">
                  <motion.span
                    ...
                    className="absolute left-4 top-4 z-10 block h-3 w-3 -translate-x-1/2 rounded-full bg-primary shadow-glow-red md:left-1/2"
                  />
                  <motion.div
                  ...
                  >
                    <article className="group rounded-xl border border-white/5 bg-surface-2/80 p-6 backdrop-blur transition hover:border-primary/30">
                      ...
                      <p className="text-sm text-text-muted">
                        <span className="text-foreground">{exp.role}</span> · {exp.company}
                      </p>
                      ...
                      <div className="mt-5 flex flex-wrap gap-1.5">
                        {exp.stack.map((s) => (
                          <span
                            key={s}
                            className="rounded border border-white/5 bg-surface-3 px-2 py-0.5 font-mono text-[11px] text-secondary"
                          >
                            {s}
                          </span>
                        ))}
