...
export function Hero() {
  return (
    <section id="top" className="relative flex min-h-screen items-center overflow-hidden bg-background">
      ...
      <svg aria-hidden viewBox="0 0 1440 320" className="pointer-events-none absolute bottom-0 left-0 w-full opacity-[0.07]">
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl px-6 pt-32 pb-20">
        <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="font-mono-tag mb-6 text-primary">
          // Engineer in transit
        </motion.p>
        ...
        <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.5 }} className="mt-8 max-w-xl text-base leading-relaxed text-text-muted md:text-lg">
          Engineering connected systems across AI, cloud and automation — fast, precise, and built to respond.
        </motion.p>

        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.05, duration: 0.5 }} className="mt-10 flex flex-wrap items-center gap-3">
          ...
          <a href={CONTACT.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn" className="rounded-md border border-white/10 p-3 text-foreground/80 transition hover:scale-105 hover:border-primary/60 hover:text-primary">
            <Linkedin className="h-4 w-4" />
          </a>
        </motion.div>
      </div>

      <motion.a href="#about" aria-label="Scroll down" initial={{ opacity: 0 }} animate={{ opacity: 1, y: [0, 6, 0] }} transition={{ delay: 1.4, duration: 2, repeat: Infinity, ease: "easeInOut" }} className="absolute bottom-8 left-1/2 z-10 -translate-x-1/2 text-text-muted">
