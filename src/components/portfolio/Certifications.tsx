      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {CERTIFICATIONS.map((c, i) => {
            ...
            return (
              <motion.a key={c.name} href={c.url} target="_blank" rel="noreferrer" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ delay: i * 0.05 }} className="group flex items-center gap-4 rounded-lg border border-white/5 bg-surface-2/70 px-5 py-4 backdrop-blur transition hover:border-primary/40">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-surface-3 text-primary"><Icon className="h-5 w-5" /></div>
