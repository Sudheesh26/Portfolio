      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {SKILL_CATEGORIES.map((cat, idx) => {
            ...
            return (
              <motion.div key={cat.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ delay: idx * 0.08 }} onMouseEnter={() => setActive(idx)} onMouseLeave={() => setActive(null)} className={`group relative overflow-hidden rounded-2xl border bg-surface-2/70 p-8 backdrop-blur transition ${isActive ? "border-primary/40" : "border-white/5"}`}>
                ...
                <div className="relative">
                  ...
                  <svg viewBox="0 0 480 280" className="mb-5 h-72 w-full animate-float">
                    {cat.children.map((_, i) => {
                      ...
                      return <motion.line key={`l-${i}`} x1={240} y1={140} x2={240 + Math.cos(angle) * 160} y2={140 + Math.sin(angle) * 95} stroke={isActive ? "rgb(220 38 38 / 0.55)" : "rgb(255 255 255 / 0.1)"} strokeWidth="0.8" initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: i * 0.05 }} />;
                    })}
                    ...
                    {cat.children.map((c, i) => {
                      ...
                      return (
                        <g key={`n-${i}`}>
                          ...
                          <text x={x} y={y - 14} textAnchor="middle" className="fill-foreground/90" style={{ font: "bold 14px var(--font-mono)" }}>{c}</text>
                        </g>
                      );
                    })}
                  </svg>
                  <div className="flex flex-wrap gap-2">
                    {cat.children.map((c) => <span key={c} className="rounded border border-white/5 bg-surface-3 px-2.5 py-1 font-mono text-xs font-bold text-secondary">{c}</span>)}
                  </div>
                </div>
              </motion.div>
            );
